...
def createNetwork():
    ...
    W_fc1 = weight_variable([32, 64])
    b_fc1 = bias_variable([64])

    W_fc2 = weight_variable([64, 4])
    ...
    h_conv3_flat = tf.reshape(s, [-1, 32])
...
checkpoint = tf.train.get_checkpoint_state("saved_networks3")
...
t = 0
s_t = np.stack((TABLE,TABLE),axis=2)
s_tr = np.stack((TABLE,TABLE),axis=2)
maxScore = 0
    ...
    while True:
        ...
        if 1==1:
            if running:
                ...
                s_tr1 = np.stack((new_table, s_tr[:,:,0]), axis=2)
                s_t1 = np.stack((TABLE, s_t[:,:,0]), axis=2)
                ...
                if t % 10000 == 0:
                    saver.save(sess, 'saved_networks3/py2048', global_step = t)
